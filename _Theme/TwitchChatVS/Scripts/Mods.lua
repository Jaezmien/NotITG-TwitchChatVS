...
-- Waits before hiding
--function JudgmentTween(self) self:zoom(1.2) self:bounceend(0.2) self:zoom(.75) self:sleep(.6) self:accelerate(.2) self:zoom(0) end
-- Instant Hide
...
judgmentFontList = NOTESKIN:GetNoteSkinNames()
...
for pn = 1,2 do local px = Screen():GetChild('PlayerP'..pn) if px then px = px:GetChild('Judgment'):GetChild(''); px:aux(pn); if ModCustom and ModCustom.JudgmentFont and ModCustom.JudgmentFont[pn] ~= 1 then px:Load( THEME:GetPath( EC_GRAPHICS, '', '_Judgments/'..judgmentFontList[ModCustom.JudgmentFont[pn]] )) end end end
end
...
if fnctn == 'Decompress' then
	...
	if Player(pn) and ModCustom and ModCustom.Compare and (ModCustom.Compare[pn] == 2 or ModCustom.Compare[pn] == 3 or ModCustom.Measure[pn] ~= 1) then
...
if add then
	...
	if not AllScores then AllScores = {} end
...
for pn = 1,2 do if Player(pn) then local t = Profile(pn)
	...
	if ModCustom then for s,v in pairs(ModCustom) do t.Mods[s] = v[pn] end end
	if vocalize then Profile(pn).Voice = vocalize[pn] end
	if ModCustom then t.Mods.JudgmentFont = judgmentFontList[ModCustom.JudgmentFont[pn]] end
...
if _G[str] then
	if _G[str][1] or not Player(1) then _G[str][2] = self; self:aux(2) else _G[str][1] = self; self:aux(1) end
end
end
...
ModsMaster.Turn =			{ modlist = {'Mirror','SoftShuffle','SpookyShuffle','Blender','SmartBlender'}, default = 'no mirror,no left,no right,no shuffle,no supershuffle,no softshuffle, no spookyshuffle, no smartblender', mods = {'mirror','softshuffle','spookyshuffle','supershuffle','smartblender'} }
...
ModsMaster.DrawSize =       { float = true }
ModsMaster.SuddenOffset =   { float = true }
...
for pn=1,2 do if ModCustom and ModCustom.LifeBar and ModCustom.LifeBar[pn] == 2 then
...
if pcall(DifficultyListTEMP) then
